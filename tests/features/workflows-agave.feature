Feature: Workflows 
  ...
  Scenario: Workflows with inputs that have spaces in the path run successfully in Agave.
    Given The "agave" "rotate2" workflow has been installed
    When I run the "agave" "rotate2" workflow with the following inputs and parameters
        | type  | name         | value                  |
        | input | input_folder | data/data with spaces  |
    Then The "agave" "rotate2" workflow "rotate_3" step produces an output file called "test 1_rot1_rot2b_rot3.txt" with the following contents
        | line   |
        | Nkrru  |
        | Cuxrj! |
